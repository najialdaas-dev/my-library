...
export async function POST(request: NextRequest) {
  try {
    ...
    // Generate safe, sanitized and unique filename
    const uniqueSuffix = Date.now() + '-' + Math.round(Math.random() * 1e9)
    const safeBaseName = fileName
      .substring(0, fileName.lastIndexOf('.'))
      .replace(/[^a-zA-Z0-9\u0600-\u06FF-]/g, '_') // keeps english, arabic letters, numbers, dashes

    const uniqueFileName = `${safeBaseName}-${uniqueSuffix}${fileExtension}`
